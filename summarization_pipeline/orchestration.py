...
class Extractor:
    ...
    def summarize(self, section_name, section_text, max_tokens):
        summarize_sys_prompt = 'You are a tool that summarizes the given text. The given text is a section of an article. Give a concise summary of the section text to include only the most important information. Give the output using maximum of ' + str(4/3 * max_tokens) + ' words. The sentences should not be left unfinished.'
    ...
    def extract_insights(self, input, max_tokens):
        insights_sys_prompt = 'You are a tool that extracts key insights from an article. You will be provided with article sections. As an output, you should provide concise insights about the given article in bulletpoints. Give the insights using maximum of ' + str(4/3 * max_tokens) + ' words. The sentences should not be left unfinished.'
        prompt = input
